...
ProcessMaster::ProcessMaster(Application& app)
    : mApp(app)
    , mSigChild(app.getMainIOService())
{
}
...
public:
    Impl(std::shared_ptr<Timer> const& outerTimer,
        ...
        , mObjHandle(outerTimer->get_io_service(), hProcess)
        {
            auto ot = mOuterTimer;
            auto oe = mOuterEc;
            mObjHandle.async_wait(
                [hProcess, hThread, ot, oe](asio::error_code ec)
                {
...
ProcessMaster::runProcess(std::string const& cmdLine)
{
    ...
    pe.mImpl = std::make_shared<ProcessExitEvent::Impl>(pe.mTimer,
                                                        pe.mEc,
                                                        pi.hProcess,
                                                        pi.hThread);
    return pe;
...
ProcessMaster::ProcessMaster(Application& app)
    : mApp(app)
    , mSigChild(app.getMainIOService(), SIGCHLD)
{
    startSignalWait();
}
