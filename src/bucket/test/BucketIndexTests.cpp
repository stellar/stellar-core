...
class BucketIndexTest
{
  protected:
    ...
    // which is more than sufficient (levelShouldSpill returns true on multiples
          ...
          "[bucket][bucketindex]")
{
    auto f = [&](Config& cfg) {
        ...
        SECTION("k disjoint buckets")
        {
             ...
             * different numbers of buckets, e.g., edge cases (e.g., 0/1) and
             * non-powers of two. */
        ...
        SECTION("same bucket at non-adjacent slots with shadowing")
        {
            ...
            auto bucketB = makeBucket({}, {offers.back()}, {}, /*pad=*/false);
