...
class SearchableBucketListSnapshot : public NonMovableOrCopyable
{
  ...
  public:
    ...
    // Loads LedgerEntry from the specified historical snapshot. Returns
    // <LedgerEntryPtr, true> if the snapshot for the given ledger is available,
    // <nullptr, false> otherwise. Note that ledgerSeq is defined as the state
    // of the BucketList at the beginning of the ledger. This means that for
    // ledger N, the maximum lastModifiedLedgerSeq of any LedgerEntry in the
    // BucketList is N - 1.
