...
BucketManager::startBackgroundEvictionScan(
    SearchableSnapshotConstPtr appliedState, SorobanNetworkConfig const& cfg)
{
    releaseAssert(mSnapshotManager);
    ...
    auto ledgerSeq = appliedState->getLedgerSeq() + 1;
    auto ledgerVers = appliedState->getLedgerHeader().ledgerVersion;
    ...
    auto task = std::make_shared<task_t>(
        [appliedState, iter = cfg.evictionIterator(), ledgerSeq, ledgerVers,
         sas, &counters = mBucketListEvictionCounters,
         stats = mEvictionStatistics] {
            return appliedState->scanForEviction(ledgerSeq, counters, iter,
