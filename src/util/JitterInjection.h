...
class JitterInjector
{
  private:
    ...
    static thread_local uint64_t sInjectionCount;
    static thread_local uint64_t sDelayCount;

  public:
    struct Config
    {
        // Default probability (0.0 - 1.0) for delay injection
        // Default 1% chance of delay at each injection point
    ...
    static bool injectDelay(double probability = -1.0, uint32_t minNs = 0,
                            uint32_t maxNs = 0);
...
/**
 * Jitter injection with custom probability (0.0 - 1.0)
