...
VirtualClock::crank(bool block)
{
    ...
    size_t progressCount = 0;
    {
        ...
        // Check if there are pending actions from background threads before
        // deciding to advance virtual time. Without this check, we might
        // spuriously jump time forward while background apply results are
        ...
        if (mMode == VIRTUAL_TIME && progressCount == 0 &&
            mBackgroundWorkCount.load() == 0 && !hasPendingActions)
        {
