...
NtpWork::NtpWork(Application& app, WorkParent& parent, std::string ntpServer, std::chrono::seconds tolerance)
...
NtpWork::onRun()
{
    std::weak_ptr<NtpWork> weak = std::static_pointer_cast<NtpWork>(shared_from_this());
    mNtpClient = std::make_shared<NtpClient>(
        app().getWorkerIOService(),
        mNtpServer,
        [weak](long time){
            auto self = weak.lock();
            if (self)
    ...
    mTimeoutTimer.async_wait([weak](){
        auto self = weak.lock();
        if (self)
        {
            self->mNtpClient.reset(); // reset will call destructor which will call failure callback if needed
        }
...
{
    auto localNow =  std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch());
