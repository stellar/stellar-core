...
template<typename T>
using Verify = std::function<void(T,T,T,T,T)>;

template<typename T>
class BigDivideTester
{
public:
            ...
            for (auto b : mValues)
                for (auto c : mValues)
                    if (c != 0 && p(uint128_t{a}, uint128_t{b}, uint128_t{c}))
                        mVerify(a, b, c,
                                static_cast<uint64_t>(downResult(uint128_t{a}, uint128_t{b}, uint128_t{c})),
                                static_cast<uint64_t>(upResult(uint128_t{a}, uint128_t{b}, uint128_t{c})));
    }
...
{
    auto verifySignedUp = [](int64_t a, int64_t b, int64_t c, int64_t expectedResult){
    ...
    auto verifySignedDown = [](int64_t a, int64_t b, int64_t c, int64_t expectedResult){
    ...
    auto verifySigned = [&](int64_t a, int64_t b, int64_t c, int64_t expectedRoundedDownResult, int64_t expectedRoundedUpResult){
    ...
    auto verifyUnsignedUp = [](uint64_t a, uint64_t b, uint64_t c, uint64_t expectedResult){
        auto unsignedResult = uint64_t{};
        REQUIRE(bigDivide(unsignedResult, a, b, c, ROUND_UP));
        REQUIRE(unsignedResult == expectedResult);
    };

    auto verifyUnsignedDown = [](uint64_t a, uint64_t b, uint64_t c, uint64_t expectedResult){
        auto unsignedResult = uint64_t{};
        REQUIRE(bigDivide(unsignedResult, a, b, c, ROUND_DOWN));
        REQUIRE(unsignedResult == expectedResult);
    };

    auto verifyUnsigned = [&](uint64_t a, uint64_t b, uint64_t c, uint64_t expectedRoundedDownResult, uint64_t expectedRoundedUpResult){
    ...
    auto signedValues = std::vector<int64_t>{0, 1, 2, 3, 1000,
        INT32_MAX - 1, INT32_MAX,
        INT64_MAX - 1, INT64_MAX};
    auto unsignedValues = std::vector<uint64_t>{0, 1, 2, 3, 1000,
        INT32_MAX - 1, INT32_MAX, UINT32_MAX - 1, UINT32_MAX,
        INT64_MAX - 1, INT64_MAX,
        UINT64_MAX - 1, UINT64_MAX};

    auto signedTester = BigDivideTester<int64_t>{signedValues, verifySigned};
    auto unsignedTester = BigDivideTester<uint64_t>{unsignedValues, verifyUnsigned};
    ...
    SECTION("upper limits")
    {
        ...
        verifyUnsigned(UINT64_MAX, UINT64_MAX, UINT64_MAX, UINT64_MAX, UINT64_MAX);
    }
