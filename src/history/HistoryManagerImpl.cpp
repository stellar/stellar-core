...
{
    // Delete published files for all checkpoints <= ledgerSeq, working
    // backwards
    uint32_t currentCheckpoint = checkpointContainingLedger(ledgerSeq, cfg);
    ...
    while (true)
    {
        ...
        if (!filesExist && currentCheckpoint < ledgerSeq)
        {
