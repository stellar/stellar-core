...
TEST_CASE("SCP checkpoint", "[catchup][herder]")
{
    ...
    for (auto* cfg : {&cfg1, &cfg2, &cfg3})
    {
        ...
        cfg->ARTIFICIALLY_SET_CLOSE_TIME_FOR_TESTING = 2000;
...
TEST_CASE("soroban txs each parameter surge priced", "[soroban][herder]")
{
    ...
    auto test =
        [&](std::function<void(SorobanNetworkConfig & cfg)> tweakSorobanConfig,
            ...
            auto simulation = Topologies::core(4, 1, networkID, [&](int i) {
...
TEST_CASE("overlay parallel processing", "[herder][parallel]")
{
    ...
    SECTION("background signature validation")
    {
        ...
        simulation = Topologies::core(4, 1, networkID, [](int i) {
...
TEST_CASE("ledger state update flow with parallel apply", "[herder][parallel]")
{
    ...
    auto setupAndRunTests = [&](bool enableParallelApply) {
        auto sim =
            Topologies::core(4, 1.0, networkID, [enableParallelApply](int i) {
                Config cfg;
...
TEST_CASE("SCP message capture from previous ledger", "[herder]")
{
    ...
    auto confGen = [](int i) {
        auto cfg = getTestConfig(i);
        cfg.ARTIFICIALLY_ACCELERATE_TIME_FOR_TESTING = true;
        cfg.ARTIFICIALLY_SET_CLOSE_TIME_FOR_TESTING = 5000;
        return cfg;
    };
    ...
    // Examine scphistory tables for A and B for ledger 2. Each node has
    // either 1 CONFIRM and 1 EXTERNALIZE (it externalized first, having seen
    // the other's CONFIRM) or 2 EXTERNALIZEs (it saw the other's EXTERNALIZE
    // first). Over the loopback overlay exactly one node was in each state;
    // with real networking both may land in the same state, so check each
    // node independently.
    auto validSlot2History = [&](Application::pointer node) {
        return checkSCPHistoryEntries(node, 2, expConfExt) ||
               checkSCPHistoryEntries(node, 2, exp2Ext);
    };
    REQUIRE(validSlot2History(A));
    REQUIRE(validSlot2History(B));
...
TEST_CASE("late joining node reaches consensus", "[herder]")
{
    ...
    auto simulation = std::make_shared<Simulation>(networkID, [](int i) {
        auto cfg = getTestConfig(i);
...
{
    auto networkID = sha256(getTestConfig().NETWORK_PASSPHRASE);
    auto simulation = Topologies::core(4, 0.75, networkID, [&](int i) {
        auto cfg = getTestConfig(i, Config::TESTDB_DEFAULT);
        cfg.EXPERIMENTAL_PARALLEL_TX_SET_DOWNLOAD = true;
    ...
    std::optional<uint32_t> startingProtocol = std::nullopt)
{
    ...
    auto simulation =
        Topologies::separateAllHighQuality(4, networkID, [&](int i) {
            auto cfg = getTestConfig(i, Config::TESTDB_DEFAULT);
...
TEST_CASE("prepare nomination before trigger", "[herder][early-nomination]")
{
    ...
    auto const expectedConsensusTime = ConsensusTime::fromSystemTime(
        clock.system_now() +
            app->getLedgerManager().getExpectedLedgerCloseTime(),
        lcl.header.ledgerVersion);
    auto const expectedCloseTime =
        expectedConsensusTime.toApplyTime().timePoint();
    ...
    SECTION("trigger reuses exactly the validated set and close time")
    {
        ...
        REQUIRE(applicable);
        auto offset =
            preparedCloseTime.toApplyTime() - getApplyTime(lcl.header.scpValue);
        REQUIRE(
            applicable->checkValid(*app, offset.seconds(), offset.seconds()));
        REQUIRE(!applicable->checkValid(*app, offset.seconds() + 1,
                                        offset.seconds() + 1));
        testutil::crankUntil(
            app,
            [&]() {
...
TEST_CASE("ms close times support subsecond ledgers", "[herder]")
{
    ...
    // same whole second
    auto sim = Topologies::core(3, 1.0, networkID, [](int i) {
        auto cfg = getTestConfig(i, Config::TESTDB_DEFAULT);
          ...
          "[upgrades][herder][acceptance]")
{
    ...
    // Stop the fourth node with its database intact before the upgrade, then
    // restart it below to exercise replay across the boundary over overlay v2.
    auto const laggingSeq = lclSeq(3);
    simulation->removeNode(keys[3].getPublicKey());
    ...
    // Restart the lagging node from its pre-upgrade database.
    simulation->addNode(keys[3], qSet, &configs[3], false)->start();
...
TEST_CASE("upgrade scheduling under sub-second ledgers", "[upgrades]")
{
    ...
    LedgerUpgrade expected(LEDGER_UPGRADE_BASE_FEE);
    expected.newBaseFee() = header.baseFee * 2;
    REQUIRE(due == std::vector<LedgerUpgrade>{expected});
