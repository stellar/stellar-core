...
namespace
{
TransactionFrameBasePtr transaction(Application& app, TestAccount& account,
                                    int64_t sequenceDelta, int64_t amount,
                                    uint32_t fee, bool isSoroban,
                                    int nbOps = 1) = delete;
          ...
          "[herder][transactionqueue]")
{
    ...
    SECTION("add high fee tx with eviction")
    {
        ...
        SECTION("then cannot add tx with lower fee than evicted")
        {
            ...
            REQUIRE(nextTx->getResult().feeCharged == 301);
        }
