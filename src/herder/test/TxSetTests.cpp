...
TEST_CASE("generalized tx set XDR conversion", "[txset]")
{
    ...
    SECTION("multiple component sets")
    {
        ...
        REQUIRE(comps[1].txsDiscountedFee().baseFee == 123);
        REQUIRE(comps[1].txsDiscountedFee().txs.size() == 1);
        REQUIRE(comps[2].txsDiscountedFee().baseFee == 1234);
        REQUIRE(comps[2].txsDiscountedFee().txs.size() == 2);
        REQUIRE(comps[3].txsDiscountedFee().baseFee == 12345);
        REQUIRE(comps[3].txsDiscountedFee().txs.size() == 3);
    }
