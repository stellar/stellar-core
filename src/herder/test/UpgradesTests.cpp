...
//     auto baseFeeUpgrade =
//     makeBaseFeeUpgrade(cfg.TESTING_UPGRADE_DESIRED_FEE); auto txCountUpgrade
//     =
...
// void
// testValidateUpgrades(VirtualClock::system_time_point
// preferredUpgradeDatetime,
//                      bool canBeValid)
// {
...
//                     ledgerUpgradeType, nomination, *app));
//             }
//             // 10 is queued, so this upgrade is only valid when not
//             nominating bool v9Upgrade = Upgrades{cfg}.isValid(
//                 toUpgradeType(makeProtocolVersionUpgrade(9)),
//                 ledgerUpgradeType, nomination, *app);
//             if (nomination)
//             {
//                 REQUIRE(!v9Upgrade);
...
//             REQUIRE(!Upgrades{cfg}.isValid(
//                 toUpgradeType(makeProtocolVersionUpgrade(7)),
//                 ledgerUpgradeType, nomination, *app));
...
//     LedgerManager::GENESIS_LEDGER_VERSION); REQUIRE(lcl.header.baseFee ==
//     LedgerManager::GENESIS_LEDGER_BASE_FEE); REQUIRE(lcl.header.maxTxSetSize
//     ==
...
//     SECTION("base fee")
//     {
//         REQUIRE(executeUpgrade(*app, makeBaseFeeUpgrade(1000)).baseFee ==
//         1000);
//     }
...
//                     SCVal val;
//                     val.type(SCV_BYTES);
//                     val.bytes().insert(val.bytes().begin(),
//                     contractID.begin(),
//                                        contractID.end());
...
//                     le.data.contractData().contract.contractId() =
//                     contractID; le.data.contractData().durability =
//                     PERSISTENT; le.data.contractData().key = key;
...
//             Upgrades::UpgradeParameters scheduledUpgrades;
//             LedgerTxn ltx(app->getLedgerTxnRoot());
//             configUpgradeSet = makeParallelComputeUpdgrade(ltx,
//             clusterCount);

//             scheduledUpgrades.mUpgradeTime = genesis(0, 1);
//             scheduledUpgrades.mConfigUpgradeSetKey =
//             configUpgradeSet->getKey();
//             app->getHerder().setUpgrades(scheduledUpgrades);
//             ltx.commit();
//         }
//         LedgerTxn ltx(app->getLedgerTxnRoot());
...
// TEST_CASE("SCP timing config affects consensus behavior",
// "[upgrades][herder]")
// {
//     auto networkID = sha256(getTestConfig().NETWORK_PASSPHRASE);
//     auto simulation =
...
//             // Allow a few ledgers of error since ledger times are not
//             absolute REQUIRE(abs(actualLedgerCount - expectedLedgers) <= 2);
//         };
...
//         REQUIRE(initialConfig.nominationTimeoutInitialMilliseconds() ==
//         1000); REQUIRE(initialConfig.nominationTimeoutIncrementMilliseconds()
//         == 1000); REQUIRE(initialConfig.ballotTimeoutInitialMilliseconds() ==
//         1000); REQUIRE(initialConfig.ballotTimeoutIncrementMilliseconds() ==
//         1000);
...
//         auto nomTimeout1 = scpDriver.computeTimeout(1,
//         /*isNomination=*/true); REQUIRE(nomTimeout1 ==
//         std::chrono::milliseconds(1000)); auto nomTimeout5 =
//         scpDriver.computeTimeout(5, /*isNomination=*/true);
...
//         REQUIRE(timeout5 == std::chrono::milliseconds(5900)); // 1500 +
//         4*1100
...
//         REQUIRE(nomTimeout5 == std::chrono::milliseconds(5000)); // 2000 +
//         4*750
//     }
// }
...
//         // transactions, or by multiplicatively increasing the memory cost,
//         so
...
//             });
//         ensureInMemorySizeIncreased();
//         // There is a small expected state size diff due to the settings
//         upgrade
//         // contract.
...
//             configUpgradeSet = makeMaxContractSizeBytesTestUpgrade(ltx2,
//             32768); ltx2.commit();
//         }
...
//             configEntry.contractCompute().feeRatePerInstructionsIncrement =
//             111; configEntry.contractCompute().txMemoryLimit =
...
//             LedgerTxn ltx2(app->getLedgerTxnRoot());
//             configUpgradeSet = makeConfigUpgradeSet(ltx2,
//             configUpgradeSetXdr); ltx2.commit();
//         }
//         executeUpgrade(*app, makeConfigUpgrade(*configUpgradeSet));
...
//     auto issuer = root->create("issuer", lm.getLastMinBalance(0) + 100 *
//     txFee); auto native = txtest::makeNativeAsset(); auto cur1 =
//     issuer.asset("CUR1"); auto cur2 = issuer.asset("CUR2");
...
//         {
//             LedgerTxn ltx(app->getLedgerTxnRoot());
//             auto trust = stellar::loadTrustLine(ltx, acc.getPublicKey(),
//             asset); res.selling =
//             trust.getSellingLiabilities(ltx.loadHeader()); res.buying =
//             trust.getBuyingLiabilities(ltx.loadHeader());
//         }
//         return res;
//     };
...
//         {
//             auto a1 =
//                 root->create("A", lm.getLastMinBalance(5) + 2000 + 5 *
//                 txFee);
...
//             REQUIRE(getAssetLiabilities(a1, cur1) == Liabilities{4000,
//             2000});
//         }
...
//         {
//             auto a1 =
//                 root->create("A", lm.getLastMinBalance(5) + 1000 + 5 *
//                 txFee);
//             a1.changeTrust(cur1, 6000);
//             issuer.pay(a1, cur1, 2000);
...
//                 [&](TestAccount& acc, Asset const& selling, Asset const&
//                 buying,
//                     int64_t quantity, std::vector<TestMarketOffer>& offers,
//                     OfferState const& afterUpgrade = OfferState::SAME) {
//                     OfferState state = {selling, buying, Price{2, 1},
//                     quantity}; auto offer = market.requireChangesWithOffer(
//                         {}, [&] { return market.addOffer(acc, state); });
//                     if (afterUpgrade == OfferState::SAME)
//                     {
//                         offers.push_back({offer.key, offer.state});
...
//             createOffer(a1, native, cur1, offers);
//             createOffer(a1, native, cur1, offers);
//             createOfferQuantity(a1, cur1, native, INT64_MAX / 4 - 2000,
//             offers,
//                                 OfferState::DELETED);
//             createOfferQuantity(a1, cur1, native, INT64_MAX / 4 - 2000,
//             offers,
...
//             market.requireChanges(offers, executeUpgrade);
//             REQUIRE(getAssetLiabilities(a1, cur1) == Liabilities{4000,
//             2000}); REQUIRE(getAssetLiabilities(a1, cur2) ==
...
//         {
//             auto a1 =
//                 root->create("A", lm.getLastMinBalance(14) + 4000 + 14 *
//                 txFee);
...
//             REQUIRE(getAssetLiabilities(a1, cur1) == Liabilities{8000,
//             4000}); REQUIRE(getAssetLiabilities(a1, cur2) ==
...
//         {
//             auto a1 =
//                 root->create("A", lm.getLastMinBalance(14) + 2000 + 14 *
//                 txFee);
//             a1.changeTrust(cur1, 12000);
//             a1.changeTrust(cur2, 12000);
//             issuer.pay(a1, cur1, 4000);
...
//             auto a2 =
//                 root->create("B", lm.getLastMinBalance(14) + 4000 + 14 *
//                 txFee);
...
//             REQUIRE(getAssetLiabilities(a1, cur1) == Liabilities{8000,
//             4000}); REQUIRE(getAssetLiabilities(a1, cur2) ==
//             Liabilities{8000, 4000}); REQUIRE(getLiabilities(a2) ==
//             Liabilities{8000, 4000}); REQUIRE(getAssetLiabilities(a2, cur1)
//             == Liabilities{8000, 4000}); REQUIRE(getAssetLiabilities(a2,
//             cur2) == Liabilities{8000, 4000});
//         }
...
//             REQUIRE(getAssetLiabilities(a1, cur1) == Liabilities{4000,
//             4000}); REQUIRE(getAssetLiabilities(a1, cur2) ==
//             Liabilities{4000, 4000}); REQUIRE(getLiabilities(a2) ==
//             Liabilities{4000, 4000}); REQUIRE(getAssetLiabilities(a2, cur1)
//             == Liabilities{4000, 4000}); REQUIRE(getAssetLiabilities(a2,
//             cur2) == Liabilities{8000, 0});
//         }
//     }
...
//                     int64_t quantity, std::vector<TestMarketOffer>& offers,
//                     OfferState const& afterUpgrade = OfferState::SAME) {
//                     OfferState state = {selling, buying, Price{3, 2},
//                     quantity}; auto offer = market.requireChangesWithOffer(
//                         {}, [&] { return market.addOffer(acc, state); });
//                     if (afterUpgrade == OfferState::SAME)
//                     {
...
//                     int64_t quantity, std::vector<TestMarketOffer>& offers,
//                     OfferState const& afterUpgrade = OfferState::SAME) {
//                     OfferState state = {selling, buying, Price{2, 3},
//                     quantity}; auto offer = market.requireChangesWithOffer(
//                         {}, [&] { return market.addOffer(acc, state); });
//                     if (afterUpgrade == OfferState::SAME)
//                     {
...
//             auto a1 =
//                 root->create("A", lm.getLastMinBalance(10) + 2000 + 12 *
//                 txFee);
...
//         {
//             auto a1 =
//                 root->create("A", lm.getLastMinBalance(6) + 4000 + 6 *
//                 txFee);
//             a1.changeTrust(cur1, 6000);
//             a1.changeTrust(cur2, 6000);
//             issuer.allowTrust(cur1, a1);
...
//         {
//             auto a1 =
//                 root->create("A", lm.getLastMinBalance(10) + 2000 + 10 *
//                 txFee);
//             a1.changeTrust(cur1, 6000);
//             a1.changeTrust(cur2, 6000);
//             issuer.allowTrust(cur1, a1);
...
//             REQUIRE(getLiabilities(a1) == Liabilities{4000, 2000});
//             REQUIRE(getAssetLiabilities(a1, cur1) == Liabilities{4000,
//             2000}); REQUIRE(getAssetLiabilities(a1, cur2) == Liabilities{0,
//             0});
//         }
...
//     {
//         auto stranger =
//             TestAccount{*app, txtest::getAccount(fmt::format("stranger{}",
//             i))};
...
//                 {root->tx({txtest::createAccount(stranger, big)})}, *app, 0,
//                 0) .first;
...
//                 return
//                 LiveBucketInputIterator(b).getMetadata().ledgerVersion;
//             };
//             switch (ledgerSeq)
//             {
//             default:
...
// // the LedgerCloseMeta version for vN. This test checks that the meta
// versions
...
// TEST_CASE("configuration initialized in version upgrade",
// "[soroban][upgrades]")
// {
//     VirtualClock clock;
//     auto cfg = getTestConfig(0);
...
//             expectToDeleteNativeSells ? OfferState::DELETED :
//             OfferState::SAME;
...
//                 return market.updateOffer(acc, offer.key.offerID,
//                 delOfferState,
//                                           OfferState::DELETED);
//             });
//         }
...
//                                   std::bind(executeUpgrade, baseReserve /
//                                   2));
...
//             REQUIRE(getAssetLiabilities(a1, cur1) == Liabilities{8000,
//             4000}); REQUIRE(getAssetLiabilities(a1, cur2) ==
//             Liabilities{8000, 4000}); deleteOffers(a1, offers);
//         });
//     }
...
//                                        app->getAppConnector());
//             REQUIRE(
//                 tx->checkValidForTesting(app->getAppConnector(), ltx, 0, 0,
//                 0));
...
//                 submitTx(transactionFrameFromOps(app->getNetworkID(), a1,
//                 opsA1,
//                                                  {a2}));
//                 submitTx(transactionFrameFromOps(app->getNetworkID(), a2,
//                 opsA2,
//                                                  {a1}));
//             }
...
//                 REQUIRE(getLiabilities(sponsoredAcc2) == Liabilities{8000,
//                 0}); REQUIRE(getAssetLiabilities(sponsoredAcc2, cur1) ==
//                         Liabilities{4000, 4000});
...
//                                  lm.getLastMinBalance(14) + 3999 + 15 *
//                                  txFee);
...
//                         "pulled on upgrade")
//                 {
//                     accSponsorsAllOffersTest(sponsoring, sponsored,
//                     sponsored2,
//                                              true, true);
//                 }
...
//     {
//         auto nodes = std::vector<LedgerUpgradeNode>{{upgrade, genesis(0,
//         15)},
//                                                     {upgrade, genesis(0,
//                                                     15)}, {upgrade,
//                                                     genesis(0, 15)}};
...
//                                            {upgradeBadFee, genesis(0, 0)}};
//         auto checks = std::vector<LedgerUpgradeCheck>{
//             {genesis(0, 10), {expectedResult, expectedResult,
//             expectedResult}}};
//         simulateUpgrade(nodes, checks, true);
//     }
...
//     {
//         LedgerTxn ltx(app->getLedgerTxnRoot());
//         auto configUpgradeSet = makeMaxContractSizeBytesTestUpgrade(ltx,
//         32768); initUpgrades.mConfigUpgradeSetKey =
//         configUpgradeSet->getKey(); ltx.commit();
//     }
//     {
...
//       "configupgradesetkey" : {
//          "data" :
//          "A2X1x61JPcqp3xe1AxsI6w3fqehhW6iU16Tn5HV32eiPU4K5Q3ayQUPGrHt7nMSvsWFD86wQYI9P6fiJD9kI+w==",
//          "nullopt" : false
//       }
//    },
//    "expirationminutes" : {
...
//         REQUIRE(networkConfig().getAverageSorobanStateSize() ==
//         correctAverage);
//     };
...
//         executeUpgrade(*app,
//         makeProtocolVersionUpgrade(fixedProtocolVersion)); return
//         app->getAppConnector()
