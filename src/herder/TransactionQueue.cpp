...
TransactionQueue::AddResult::AddResult(AddResultCode addCode,
    ...
    : code(addCode)
    , txResult(std::shared_ptr<MutableTransactionResultBase const>(
          tx.createTxErrorResult(txErrorCode)))
{
}
...
TransactionQueue::broadcastTx(TransactionFrameBasePtr tx)
{
    ZoneScoped;
...
TransactionQueue::rebroadcast()
{
    ...
    // BroadcastSeed should have rotated already, so it's fine to use it for
    // re-sorting.
