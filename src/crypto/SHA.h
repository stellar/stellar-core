...
// HMAC-SHA256 (keyed)
HmacSha256Mac hmacSha256(OverlayStreamKey const& key, ByteSlice const& bin);

// Use this rather than HMAC-output ==, to avoid timing leaks.
bool hmacSha256Verify(HmacSha256Mac const& hmac, OverlayStreamKey const& key,
                      ByteSlice const& bin);

// Unsalted HKDF-extract(bytes) == HMAC(<zero>,bytes)
OverlayStreamKey hkdfExtract(ByteSlice const& bin);

// Single-step HKDF-expand(key,bytes) == HMAC(key,bytes|0x1)
OverlayStreamKey hkdfExpand(OverlayStreamKey const& key, ByteSlice const& bin);
}
