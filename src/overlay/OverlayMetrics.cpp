...
OverlayMetrics::OverlayMetrics(Application& app)
    ...
    , mRecvTransactionAccumulator(
          app.getMetrics().NewCounter({"overlay", "recv", "transaction-time"}))
    ...
    , mAdvertQueueDelayAccumulator(app.getMetrics().NewCounter(
          {"overlay", "flood", "advert-delay-time"}))
    , mAdvertQueueDelayCounter(app.getMetrics().NewCounter(
          {"overlay", "flood", "advert-delay-count"}))
