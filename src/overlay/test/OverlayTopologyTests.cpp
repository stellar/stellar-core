...
TEST_CASE("basic connectivity", "[overlay][connectivity][!hide]")
{
    ...
    auto makeSimulation = [&](int maxOutbound, int maxInbound, int numNodes,
                              int numWatchers) {
    ...
    auto test = [&](int maxOutbound, int maxInbound, int numNodes,
                    int numWatchers) {
        auto simulation =
            makeSimulation(maxOutbound, maxInbound, numNodes, numWatchers);
    ...
    SECTION("sparse capacity: top tier connects via mutual qset peering")
    {
        // A single ordinary outbound connection per node is far too sparse
        // for random peering to reliably connect the network (this
        // configuration was expected to fail before mutual qset peering).
        // The top tier must nonetheless wire itself into a full mesh, since
        // direct qset peer connections do not depend on ordinary capacity.
        auto simulation = makeSimulation(1, 20, topTierSize, 100);

        // The node keys generated by Topologies::separate
        std::vector<NodeID> topTier;
        for (int i = 0; i < topTierSize; ++i)
        {
            topTier.emplace_back(
                SecretKey::fromSeed(sha256("NODE_SEED_" + std::to_string(i)))
                    .getPublicKey());
        }

        auto topTierFullyConnected = [&]() {
            for (auto const& id : topTier)
            {
                auto const& authenticated = simulation->getNode(id)
                                                ->getOverlayManager()
                                                .getAuthenticatedPeers();
                for (auto const& other : topTier)
                {
                    if (!(other == id) &&
                        authenticated.find(other) == authenticated.end())
                    {
                        return false;
                    }
                }
        ...
        simulation->crankUntil(topTierFullyConnected,
                               std::chrono::seconds(300), false);
        logTopologyInfo(simulation);
        REQUIRE(topTierFullyConnected());
    }
