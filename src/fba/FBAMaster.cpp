...
Timestamp:
    set to be <min close time> seconds after your first proposal if you are
making one fresh.
...
Commit means:	I think this ballot is the correct one and I will
externalize that fact
...
Process:
        ...
        The ballot we initially prepare depends on the following:
                1) Highest from your Q set that includes all past tx and has the
most votes
        ...
        We don't want to switch to a higher ballot if it is close to being
ratified
...
FBAMaster::startNewRound(const stellarxdr::SlotBallot& firstBallot)
{
    ...
    mWaitFutureStatements.clear(); // SANITY: do these get cleaned up right away
                                   // or do we need to go through and call
                                   // cancel on all the timers?
...
4) check if we have this tx set       (LATER: We could switch the order of 4 and
5a. We will forward less messages and could inadvertently sybil people if we do
though)
...
bool
FBAMaster::processStatement(Statement::pointer statement)
{
    if (!statement->isSigValid())
        return false; // 1) yes   LATER we should doc any peer that sends us one
                      // of these

    TxHerderGateway::SlotComparisonType slotCompare =
        mApp.getTxHerderGateway().compareSlot(statement->getSlotBallot());
...
void
FBAMaster::processStatements(vector<Statement::pointer>& statementList)
{
...
void
FBAMaster::addQuorumSet(QuorumSet::pointer qset)
{
