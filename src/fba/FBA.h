...
class FBA
{
  ...
  public:
    ...
    virtual Value mergeValueParts(const Value& p1, const Value& p2)
    {
        using xdr::operator<;
    ...
    // Submit a value part for the ready phase. The ready phase has no direct
    // impact on the consensus phase. It will call the callback once a value is
    // ready along with its evidence.
    bool readyValue(const uint64& slotIndex,
                    const Value& valuePart,
                    std::function<void(Value, FBAReadyEvidence)> const& cb);

    void validateReady(const uint64& slotIndex,
                       const Value& value,
  ...
  private:
    ...
    // Slot and ReadySlot getter
    ReadySlot* getReadySlot(const uint64& slotIndex);
    ...
    std::map<uint64, ReadySlot*>   mKnownReadySlots;
    std::map<uint64, Slot*>        mKnownSlots;

    friend class ReadySlot;
    friend class Slot;
    friend class Node;
};
}
