...
A is ratified if:
    ...
    Should a node keep track of who depends on it so when its vote changes it
can tell them?

    We trigger a recheck when some node flips from PLEDGING to
Q_NOT_PLEDGING_STATE and that node has already been visited
...
How we check for ratification:
    ...
    It asks all of its Qset what their state is
        Each node recursively checks.
            ...
            If a node doesn't have a Q then we increment the recheckCounter and
start the recursive search over
...
//  it will increment anytime a node is removed from Va thus triggering a
//  recheck of all nodes in the "pledging" state
                    ...
                    Application& app)
{
    ...
    { // this node was already visited during this check
        if (recheckCounter == mRecheckCounter || mRatState != PLEDGING_STATE)
            return mRatState;
    }
    else
    {
        ...
        { // check if this guy has already pledged a stronger
            ...
            if (ourStatement)
            {
                ...
                { // if we are pledging a
                    ...
                    mRatState = NOTPLEDGING_STATE; // SANITY is this correct to
                                                   // do this? We have pledged a
                                                   // different ballot at a
                                                   // higher level.
                // is it safe to say we are no longer pledging this lower level
                // ballot?
                return mRatState;
            }
        }
    ...
    QuorumSet::pointer qset = app.getOverlayGateway().fetchQuorumSet(
...
bool
Node::hasStatement(StatementPtr statement)
{
...
bool
Node::addStatement(StatementPtr statement)
{
    vector<StatementPtr>& v = mStatements[statement->getType()];
    if (find(v.begin(), v.end(), statement) == v.end())
...
Node::getHighestStatement(stellarxdr::FBAStatementType type)
{
    ...
    for (auto statement : mStatements[type])
    {
        if (max)
        {
            if (ballot::compare(statement->getBallot(), max->getBallot()))
                max = statement;
        }
        else
            max = statement;
