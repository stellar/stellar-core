// Copyright 2014 Stellar Development Foundation and contributors. Licensed
// under the Apache License, Version 2.0. See the COPYING file at the root
// of this distribution or at http://www.apache.org/licenses/LICENSE-2.0

#include "crypto/SHA.h"
#include "item/ItemFetcher.h"
#include "main/ApplicationImpl.h"
#include "test/test.h"

#include <lib/catch.hpp>

using namespace stellar;

TEST_CASE("ItemFetcher fetches", "[overlay][ItemFetcher]")
{
    Config cfg(getTestConfig());
    VirtualClock clock;
    ApplicationImpl app{clock, cfg};
    app.initialize();

    auto h1 = ItemKey{ItemType::TX_SET, sha256(ByteSlice("h1"))};
    auto h2 = ItemKey{ItemType::TX_SET, sha256(ByteSlice("h2"))};
    auto h3 = ItemKey{ItemType::TX_SET, sha256(ByteSlice("h3"))};
    auto h4 = ItemKey{ItemType::TX_SET, sha256(ByteSlice("h4"))};

    std::vector<Hash> received;

    SECTION("empty")
    {
        ItemFetcher itemFetcher(app);

        REQUIRE(!itemFetcher.isFetching(h1));
        REQUIRE(!itemFetcher.isFetching(h2));
        REQUIRE(!itemFetcher.isFetching(h3));
        REQUIRE(!itemFetcher.isFetching(h4));
    }

    SECTION("fetch some items")
    {
        ItemFetcher itemFetcher(app);

        itemFetcher.fetch(nullptr, h2);
        itemFetcher.fetch(nullptr, h3);
        itemFetcher.fetch(nullptr, h3);

        REQUIRE(!itemFetcher.isFetching(h1));
        REQUIRE(itemFetcher.isFetching(h2));
        REQUIRE(itemFetcher.isFetching(h3));
        REQUIRE(!itemFetcher.isFetching(h4));
    }

    SECTION("stop fetching some items")
    {
        ItemFetcher itemFetcher(app);

        itemFetcher.fetch(nullptr, h2);
        itemFetcher.fetch(nullptr, h3);
        itemFetcher.fetch(nullptr, h3);
        itemFetcher.stopFetch(h3);

        REQUIRE(!itemFetcher.isFetching(h1));
        REQUIRE(itemFetcher.isFetching(h2));
        REQUIRE(!itemFetcher.isFetching(h3));
        REQUIRE(!itemFetcher.isFetching(h4));
    }
}
