...
{
    std::vector<uint32_t> initialNominationTimeouts = {
        MinimumSorobanNetworkConfig::NOMINATION_TIMEOUT_INITIAL_MILLISECONDS,
        InitialSorobanNetworkConfig::NOMINATION_TIMEOUT_INITIAL_MILLISECONDS,
        MaximumSorobanNetworkConfig::NOMINATION_TIMEOUT_INITIAL_MILLISECONDS,
    };

    std::vector<uint32_t> initialBallotTimeouts = {
    ...
    std::vector<uint32_t> nominationIncrements = {
        MinimumSorobanNetworkConfig::NOMINATION_TIMEOUT_INCREMENT_MILLISECONDS,
        InitialSorobanNetworkConfig::NOMINATION_TIMEOUT_INCREMENT_MILLISECONDS,
        MaximumSorobanNetworkConfig::NOMINATION_TIMEOUT_INCREMENT_MILLISECONDS,
    };

    std::vector<uint32_t> ballotIncrements = {
    ...
    for (auto initialNominationTimeout : initialNominationTimeouts)
    {
        for (auto initialBallotTimeout : initialBallotTimeouts)
        {
            for (auto nominationIncrement : nominationIncrements)
            {
                for (auto ballotIncrement : ballotIncrements)
                {
                    std::string sectionHeader = fmt::format(
                        "initialNominationTimeout: {}, initialBallotTimeout: "
                        "{}, nominationIncrement: {}, ballotIncrement: {}",
                        initialNominationTimeout, initialBallotTimeout,
                        nominationIncrement, ballotIncrement);

                    SECTION(sectionHeader)
                    {
                        scp.mInitialNominationTimeoutMS =
                            initialNominationTimeout;
                        scp.mInitialNominationTimeoutMS =
                            initialNominationTimeout;
                        scp.mInitialBallotTimeoutMS = initialBallotTimeout;
                        scp.mIncrementNominationTimeoutMS = nominationIncrement;
                        scp.mIncrementBallotTimeoutMS = ballotIncrement;
                        f(scp);
                    }
                }
            }
