...
{
    benchmarkLimitsIterationTimed();
}

double
ApplyLoad::benchmarkLimitsIterationTimed()
{
...
ApplyLoad::findMaxLimitsForModelTransaction()
{
    ...
    auto benchmarkFunc = [this](uint32_t testTxsPerLedger) -> double {
        double closeTime = benchmarkLimitsIterationTimed();
...
ApplyLoad::findMaxSacTps()
{
    ...
    uint32_t minTxRateSteps = std::max(
    ...
    auto prepareIter = [this, txsPerStep](uint32_t txRateSteps) {
        uint32_t testTxRate = txRateSteps * txsPerStep;
    ...
    auto benchmarkFunc = [this, txsPerStep](uint32_t txRateSteps) -> double {
        uint32_t testTxRate = txRateSteps * txsPerStep;
        uint32_t txsPerLedger =
            testTxRate / mApp.getConfig().APPLY_LOAD_BATCH_SAC_COUNT;
