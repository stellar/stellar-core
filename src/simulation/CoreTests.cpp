...
// Simulations run in real time on the Rust overlay (there is no virtual-time
// loopback mode), so accelerate ledger close for the topology tests.
static Config
acceleratedConfig(int configNum)
{
    auto cfg = getTestConfig(configNum);
...
TEST_CASE("3 nodes 2 running threshold 2", "[simulation][core3][acceptance]")
{

    {
        ...
        Simulation::pointer simulation =
            std::make_shared<Simulation>(networkID, acceleratedConfig);
...
// nodes never validate each other's envelopes. Re-enable once quorum-set
// fetching exists.
TEST_CASE("asymmetric topology report cost", "[simulation][!hide]")
{
    ...
    Hash networkID = sha256(getTestConfig().NETWORK_PASSPHRASE);
    Simulation::pointer simulation =
        Topologies::asymmetric(networkID, acceleratedConfig);
    simulation->startAllNodes();
          ...
          "[simulation][acceptance]")
{
    ...
    {
        Simulation::pointer sim =
            Topologies::core(size, 1.0, networkID, acceleratedConfig);
        sim->startAllNodes();
...
TEST_CASE("resilience tests", "[resilience][simulation][!hide]")
{
    ...
    auto confGen = [](int configNum) -> Config {
        ...
        c.ARTIFICIALLY_ACCELERATE_TIME_FOR_TESTING = true;
        return c;
    };

    SECTION("custom-A")
    {
        resilienceTest(Topologies::customA(networkID, confGen, 2));
    }
    SECTION("hierarchical")
    {
        resilienceTest(
            Topologies::hierarchicalQuorum(2, networkID, confGen, 2));
    }
    SECTION("simplified hierarchical")
    {
        resilienceTest(Topologies::hierarchicalQuorumSimplified(4, 3, networkID,
                                                                confGen, 2));
    }
    SECTION("core4")
    {
    ...
    {
        resilienceTest(Topologies::branchedcycle(5, 0.6, networkID, confGen));
    }
}

static void
hierarchicalTopoTest(int nLedgers, int nBranches, Hash const& networkID)
{
    LOG_DEBUG(DEFAULT_LOG, "starting topo test {} : {}", nLedgers, nBranches);

    Simulation::pointer sim =
        Topologies::hierarchicalQuorum(nBranches, networkID, acceleratedConfig);
    sim->startAllNodes();
...
TEST_CASE("hierarchical topology scales 1 to 3", "[simulation][acceptance]")
{
    ...
    auto test = [&]() {
        ...
        {
            hierarchicalTopoTest(nLedgers, nBranches, networkID);
        }
    };
    test();
}
                           ...
                           Hash const& networkID, Simulation::ConfigGen confGen)
{
    ...
    Simulation::pointer sim = Topologies::hierarchicalQuorumSimplified(
        nbCore, nbOuterNodes, networkID, confGen);
    sim->startAllNodes();
...
{
    Hash networkID = sha256(getTestConfig().NETWORK_PASSPHRASE);
    hierarchicalSimplifiedTest(4, 5, 10, networkID, acceleratedConfig);
}
...
TEST_CASE("cycle4 topology", "[simulation][.]")
{
    ...
    Hash networkID = sha256(getTestConfig().NETWORK_PASSPHRASE);
    Simulation::pointer simulation =
        Topologies::cycle4(networkID, acceleratedConfig);
    simulation->startAllNodes();
...
TEST_CASE("Mesh nodes vs network traffic", "[scalability][!hide]")
{
    netTopologyTest("mesh", [&](int numNodes) -> Simulation::pointer {
        return Topologies::core(
            numNodes, 1.0, sha256(fmt::format("nodes-{:d}", numNodes)),
            [&](int cfgNum) -> Config {
