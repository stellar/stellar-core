...
TEST_CASE("generate soroban load", "[loadgen][soroban]")
{
        ...
        SECTION("Mix with classic")
        {
            ...
            for (auto const& setting : upgrades.updatedEntry)
            {
                ...
                case CONFIG_SETTING_CONTRACT_COMPUTE_V0:
                    ...
                    break;
                case CONFIG_SETTING_CONTRACT_LEDGER_COST_V0:
                    REQUIRE(setting.contractLedgerCost()
                ...
                case CONFIG_SETTING_CONTRACT_PARALLEL_COMPUTE_V0:
                    ...
                    break;
                case CONFIG_SETTING_CONTRACT_LEDGER_COST_EXT_V0:
                    REQUIRE(setting.contractLedgerCostExt()
                                .txMaxInMemoryReadEntries ==
                            upgradeCfg.txMaxInMemoryReadEntries);
                    REQUIRE(setting.contractLedgerCostExt().feeWrite1KB ==
                            upgradeCfg.flatRateFeeWrite1KB);
                    break;
            ...
            upgradeSorobanNetworkConfig(
                [&](SorobanNetworkConfig& cfg) {
                    ...
                    // Set write limits so that we can write all keys in a
            ...
            // Check that each key is unique and exists in the DB
            // This ugly math mimics what we do in loadgen, where we calculate
            // the total number of bytes we can write, then divide the bytes
            // between the number of data entries we want to write and convert
            // this value back to kilobytes for the contract invocation. Thus we
            // need to redundantly divide then multiply by 1024 to mimic
            // rounding behavior.
            auto expectedDataEntrySize =
                ((ioKiloBytes * 1024 -
                  loadGen.getContactOverheadBytesForTesting()) /
            ...
            for (auto const& instanceKey : instanceKeys)
            {
                ...
                {
                    auto lk =
                        contractDataKey(contractID, txtest::makeU32(i),
                                        ContractDataDurability::PERSISTENT);
    ...
    TEST_CASE("Multi-op pretend transactions are valid", "[loadgen]")
    {
        ...
        try
        {
            ...
            loadGen.generateLoad(GeneratedLoadConfig::txLoad(
                LoadGenMode::PRETEND, nAccounts, 5, txRate));
    ...
    TEST_CASE("Multi-op mixed transactions are valid", "[loadgen]")
    {
        ...
        uint32_t numAccounts =
            txRate * static_cast<uint32>(
                         Herder::EXP_LEDGER_TIMESPAN_SECONDS.count() * 3);
        auto& loadGen = app.getLoadGenerator();
        loadGen.generateLoad(GeneratedLoadConfig::createAccountsLoad(
            /* nAccounts */ numAccounts,
            /* txRate */ txRate));
        try
        {
            ...
            auto config = GeneratedLoadConfig::txLoad(
                LoadGenMode::MIXED_CLASSIC, numAccounts, 100, txRate);
    ...
    TEST_CASE("Upgrade setup with metrics reset", "[loadgen]")
    {
        ...
        // Setup again. This should succeed even though it's the same account
        // with the same `runsComplete` value performing the setup
        loadgen.generateLoad(upgradeSetupCfg);
        sim->crankUntil([&]() { return runsComplete.count() == 1; },
                        5 * Herder::EXP_LEDGER_TIMESPAN_SECONDS, false);
        REQUIRE(runsFailed.count() == 0);
