...
//             /* offset*/ nAccounts,
//             cfg.LOADGEN_PREGENERATED_TRANSACTIONS_FILE));
//         simulation->crankUntil(
//             [&]() {
//                 return app.getMetrics()
//                            .NewMeter({"loadgen", "run", "complete"}, "run")
...
//         auto& acc = app.getMetrics().NewMeter({"loadgen", "account",
//         "created"},
//                                               "account");
...
//             cfg.ARTIFICIALLY_GENERATE_LOAD_FOR_TESTING = true;
//             cfg.UPDATE_SOROBAN_COSTS_DURING_PROTOCOL_UPGRADE_FOR_TESTING =
...
//         // SOROBAN_INVOKE_SETUP_LOAD should throw a helpful exception
//         explaining
//         // the misconfiguration.
//         auto invokeLoadCfg =
//             GeneratedLoadConfig::txLoad(LoadGenMode::SOROBAN_INVOKE,
//                                         /* nAccounts*/ 1, /* numSorobanTxs */
//                                         1,
//                                         /* txRate */ 1);
//         REQUIRE_THROWS_WITH(
...
//     {
//         upgradeCfg.ledgerMaxDependentTxClusters = rand_uniform<uint32_t>(2,
//         10); upgradeCfg.txMaxFootprintEntries =
//             rand_uniform<uint32_t>(UINT32_MAX - 10'000, UINT32_MAX);
...
//     total
//     // number of bytes we can write, then divide the bytes between the number
//     of
//     // data entries we want to write and convert this value back to
//     // kilobytes for the contract invocation. Thus we need to redundantly
//     divide
...
//         {
//             auto& totalFailed =
//                 node->getMetrics().NewCounter({"ledger", "apply",
//                 "failure"});
...
//             LoadGenMode::PAY, app.getConfig().GENESIS_TEST_ACCOUNT_COUNT,
//             100, txRate);
...
//     // Only a few ledger limits need to be specified, the rest will be found
//     by
