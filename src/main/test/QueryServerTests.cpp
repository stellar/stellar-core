...
TEST_CASE("getledgerentry", "[queryserver]")
{
    ...
    SECTION("lookup")
    {
        ...
        std::string retStr;
        REQUIRE(
            queryAndParse(qServer, std::nullopt, keysToSearch, root, retStr));
    ...
    SECTION("duplicate keys")
    {
        ...
        Json::Value root;
        std::string retStr;
        REQUIRE(!queryAndParse(qServer, std::nullopt, {liveEntry, liveEntry},
                               root, retStr));
    ...
    SECTION("response order matches request order")
    {
        ...
        auto testKeyOrder = [&](std::vector<LedgerKey> const& keyOrder,
            ...
            std::string retStr;
            REQUIRE(
                queryAndParse(qServer, std::nullopt, keyOrder, root, retStr));
            REQUIRE(root.isMember("entries"));
