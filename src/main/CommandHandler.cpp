...
void
CommandHandler::banaccounts(std::string const&, std::string& retStr)
{
    ZoneScoped;
    retStr =
        R"({"status": "deprecated", "warning": "banaccounts is deprecated and has no effect; account banning has been removed. See CAP-0077 (https://github.com/stellar/stellar-protocol/blob/master/core/cap-0077.md) for a more robust alternative."})";
}

void
CommandHandler::unbanaccounts(std::string const&, std::string& retStr)
{
    ZoneScoped;
    retStr =
        R"({"status": "deprecated", "warning": "unbanaccounts is deprecated and has no effect; account banning has been removed. See CAP-0077 (https://github.com/stellar/stellar-protocol/blob/master/core/cap-0077.md) for a more robust alternative."})";
}
...
CommandHandler::tx(std::string const& params, std::string& retStr)
{
    ...
    if (paramMap.find("force") != paramMap.end())
    {
        // The `force` flag used to bypass banned-account filtering, which
        // has been removed. Accept it for backwards compatibility.
        root["warning"] = "the 'force' flag is deprecated and has no effect; "
                          "account banning has been removed";
    }
