...
namespace stellar
{
    ...
    void CommandHandler::stop(const std::string& params, std::string& retStr)
    {
        retStr = "Stopping...";
        mApp.gracefulStop();
    }

    void CommandHandler::peers(const std::string& params, std::string& retStr)
    {
        retStr = "Peers...";
    }
    void CommandHandler::info(const std::string& params, std::string& retStr)
    {
        retStr = "Info...";
    }
    void CommandHandler::reloadCfg(const std::string& params, std::string& retStr)
    {
        std::string filename = params.substr(6);
        if(filename.size())
        {
            retStr = "Loading new Config file";
            // GRAYDON: do we want to call this from some other thread? mApp.mConfig.load(filename);
        } else
        {
    ...
    void CommandHandler::logRotate(const std::string& params, std::string& retStr)
    {
        retStr = "Log rotate...";
    }
    void CommandHandler::connect(const std::string& params, std::string& retStr)
    {
        retStr = "Connect to";
    }

    void CommandHandler::tx(const std::string& params, std::string& retStr)
    {
