...
class QueryServer
{
  private:
    ...
    // Per-thread cache of LedgerStateSnapshot objects. Each thread owns its
    ...
    std::unordered_map<std::thread::id, std::map<uint32_t, LedgerStateSnapshot>>
        mPerThreadSnapshots;
    ...
    std::map<uint32_t, CompleteConstLedgerStatePtr> mStates GUARDED_BY(mMutex);
    uint32_t const mMaxSnapshots;

    // Returns a cached LedgerStateSnapshot for the given ledger seq, or
