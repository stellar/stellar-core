...
// auto constexpr INITIAL_ACCOUNT_BALANCE = 1'000'000LL;    // reduced after
// setup auto constexpr INITIAL_ASSET_DISTRIBUTION = 1'000'000LL; // reduced
// after setup auto constexpr FUZZING_FEE = 1; auto constexpr FUZZING_RESERVE =
// 4; auto constexpr INITIAL_TRUST_LINE_LIMIT = 5 * INITIAL_ASSET_DISTRIBUTION;
...
// uint8_t constexpr NUMBER_OF_ASSET_CODE_BITS = 8 -
// NUMBER_OF_ASSET_ISSUER_BITS; uint8_t constexpr NUMBER_OF_ASSETS_TO_USE = 1 <<
// NUMBER_OF_ASSET_CODE_BITS; uint8_t constexpr ENCODE_ASSET_CODE_MASK =
// NUMBER_OF_ASSETS_TO_USE - 1;
...
//     // could make it so, but it's not necessary -- compacting, which alone
//     uses
...
//             return
//             getShortKey(key.contractData().contract.liquidityPoolId());
...
// // For digit != 0, returns an Asset with a 4-byte alphanumeric code
// "Ast<digit>"
...
//         beginSponsoringOp.sourceAccount.activate() =
//         toMuxedAccount(sponsorKey); ops.emplace_back(beginSponsoringOp);
//     }
...
//         endSponsoringOp.sourceAccount.activate() =
//         toMuxedAccount(sponsoredKey); ops.emplace_back(endSponsoringOp);
//     }
// }
// } // namespace
// } // namespace FuzzUtils
...
//    adjustments:
//     * use a binary representation as compact as possible, so that fuzzers
//     have
//    less data to fuzz
...
//         assert(mStart <= mEnd);
//     }
//     xdr_fuzzer_compactor(msg_ptr& m) : xdr_fuzzer_compactor(m->data(),
//     m->end())
//     {
//     }
...
//         check(2);
//         uint16_t v = static_cast<uint16_t>(xdr_traits<T>::to_uint(t) &
//         0xFFFF); put_bytes(&v, 2);
//     }
...
//     }
//     template <typename T>
//     typename std::enable_if<std::is_same<stellar::MuxedAccount,
//     T>::value>::type operator()(T const& m)
//     {
...
//         // LedgerKeys that have been mentioned in the XDR of fuzzer
//         operations. check(1); auto b =
//         stellar::FuzzUtils::getShortKey(balanceID); put_bytes(&b, 1);
//     }
...
//     {
//         // Convert LedgerKey to 1 byte for indexing into an array of
//         LedgerKeys
...
//         // If this one byte indexes a stored LedgerKey for a
//         ClaimableBalanceID,
...
//     // PoolID is just an opaque vector of size 32, so we have to specialize
//     the
...
//         if (mCur != mEnd)
//         {
//             throw xdr_bad_message_size("trailing data in
//             xdr_fuzzer_unpacker");
//         }
//     }
// };
...
//         // Do not track metrics related to background signature verification
//         in
//         // the fuzzer.
//         signatureChecker.disableCacheMetricsTracking();
//         LedgerSnapshot ltxStmt(ltx);
//         // if any ill-formed Operations, do not attempt transaction
//         application auto isInvalidOperation = [&](auto const& op, auto&
//         opResult) {
//             auto diagnostics =
...
//         // while the following method's result is not captured, regardless,
//         for
//         // protocols < 8, this triggered buggy caching, and potentially may
//         do
//         // so in the future
...
// // Handles breaking up the list of operations into multiple transactions, if
// the
...
//                     Application& app)
// {
//     auto txFramePtr = createFuzzTransactionFrame(ltx, sourceAccount, begin,
//     end,
//                                                  app.getNetworkID());
//     txFramePtr->attemptApplication(app, ltx);
// }
...
//         : TrustLineParameters(trustor, assetID,
...
// {
//     constexpr ClaimableBalanceParameters(int const sender, int const
//     claimant,
//                                          AssetID const& asset, int64_t
//                                          amount)
//         : SponsoredEntryParameters()
//         , mSender(sender)
//         , mClaimant(claimant)
...
// std::array<ClaimableBalanceParameters, 11> constexpr
// claimableBalanceParameters{
...
//                               int32_t priceNumerator, int32_t
//                               priceDenominator, bool passive)
//         : SponsoredEntryParameters()
...
//                               AssetID const& sell, int64_t amount,
//                               int32_t priceNumerator, int32_t
//                               priceDenominator, bool passive, int sponsorKey)
//         : SponsoredEntryParameters(sponsorKey)
//         , mPublicKey(publicKey)
//         , mBid(bid)
//         , mSell(sell)
...
//     {1, AssetID(1), AssetID(2), 50, 3, 2, false, 3}, // sponsored by account
//     3 {3, AssetID(1), AssetID(2), 100, 1, 1, true}, {3, AssetID(2),
//     AssetID(1), 100, 1, 1, true}, {1, AssetID(2), AssetID(1), 10, 10, 9,
//     false}, {3, AssetID(2), AssetID(1), 50, 10, 9, false}, {3, AssetID(2),
//     AssetID(1), 100, 22, 7, false},
...
//                                   int64_t maxAmountB, int32_t
//                                   minPriceNumerator, int32_t
//                                   minPriceDenominator, int32_t
//                                   maxPriceNumerator, int32_t
//                                   maxPriceDenominator, int64_t limit)
//         : SponsoredEntryParameters()
//         , mTrustor(trustor)
//         , mAssetA(assetA)
//         , mAssetB(assetB)
...
//                                   minPriceDenominator, int32_t
//                                   maxPriceNumerator, int32_t
//                                   maxPriceDenominator, int64_t limit, int
//                                   sponsorKey)
//         : SponsoredEntryParameters(sponsorKey)
//         , mTrustor(trustor)
//         , mAssetA(assetA)
...
//     // entries are returned, so to minimize non-determinism in fuzzing setup,
//     we
//     // sort them.
...
//     // If we ever create more ledger entries during setup than we have room
//     for
...
//         init.cbegin(), init.cend(), mStoredLedgerKeys.begin(),
//         LedgerEntryKey);
...
//         {
//             auto allowTrustOp =
//                 txtest::allowTrust(account, asset,
//                 trustLine.mAllowTrustFlags);
...
//         {
//             // Distribute the starting amount of the asset (to be reduced
//             after
...
//                                          param.mLimit));
//         trustOp.sourceAccount.activate() = toMuxedAccount(account);
//         FuzzUtils::emplaceConditionallySponsored(ops, trustOp,
//         param.mSponsored,
...
//             FuzzUtils::emplaceConditionallySponsored(
//                 ops, depositOp, param.mSponsored, param.mSponsorKey,
//                 account);
//         }
//     }
...
// void
// TransactionFuzzer::reduceNativeBalancesAfterSetup(AbstractLedgerTxn&
// ltxOuter)
// {
//     LedgerTxn ltx(ltxOuter);
...
//         auto const availableBalance = getAvailableBalance(ltx.loadHeader(),
//         ae); auto const targetAvailableBalance =
...
//         {
//             // Without authorization, this trustline could not have been
//             funded
...
// void
// TransactionFuzzer::reduceTrustLineLimitsAfterSetup(AbstractLedgerTxn&
// ltxOuter)
// {
//     LedgerTxn ltx(ltxOuter);
...
//     // Reduce trustline limits so that fuzzing has a better chance of
//     exercising
//     // edge cases.
//     for (auto const& trustLine : trustLineParameters)
//     {
...
//                   "xdr::xdr_from_fuzzer_opaque() threw exception {}",
//                   e.what());
//         return;
//     }
...
//     resetTxInternalState(*mApp);
//     LOG_TRACE(DEFAULT_LOG, "{}",
//               xdrToCerealString(ops, fmt::format("Fuzz ops ({})",
//               ops.size())));

//     LedgerTxn ltx(mApp->getLedgerTxnRoot());
//     applyFuzzOperations(ltx, mSourceAccountID, ops.begin(), ops.end(),
//     *mApp);
// }
...
//     auto const numops = rand_uniform<int>(1,
//     FuzzUtils::FUZZER_MAX_OPERATIONS); for (int i = 0; i < numops; ++i)
//     {
...
//         {
//             // Skip soroban txs for now because setting them up to be valid
//             will
//             // take some time.
//             continue;
//         }
...
//     mSimulation = std::make_shared<Simulation>(networkID, getFuzzConfig);
...
//     // crank until overlay is ready (with Rust overlay, Kademlia handles
//     connections) mSimulation->crankUntil(
//         [&]() {
//             auto nodes = mSimulation->getNodes();
//             // Just check that overlay managers are running
//             return
//             !nodes[ACCEPTOR_INDEX]->getOverlayManager().isShuttingDown() &&
//                    !nodes[INITIATOR_INDEX]->getOverlayManager().isShuttingDown();
//         },
