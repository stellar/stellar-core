...
initialCpuCostParamsEntry()
{
    ...
    switch (val) {
        ...
        case VmInstantiation:
            params[val] = ContractCostParamEntry{805445, 307 , ExtensionPoint{0}};
...
initialMemCostParamsEntry()
{
    ...
    switch (val) {
        ...
        case VmMemWrite:
            params[val] = ContractCostParamEntry{0, 0, ExtensionPoint{0}};
        case VmInstantiation:
            params[val] = ContractCostParamEntry{1100352, 53, ExtensionPoint{0}};
        case InvokeVmFunction:
            params[val] = ContractCostParamEntry{267, 0, ExtensionPoint{0}};
        case ChargeBudget:
            params[val] = ContractCostParamEntry{0, 0, ExtensionPoint{0}};
...
void
SorobanNetworkConfig::loadHostLogicVersion(AbstractLedgerTxn& ltx)
{
#ifdef ENABLE_NEXT_PROTOCOL_VERSION_UNSAFE_FOR_PRODUCTION
    LedgerKey key(CONFIG_SETTING);
    key.configSetting().configSettingID =
...
{
    if (params.size() != xdr::xdr_traits<ContractCostType>::enum_values().size())
    {
        return false;
    }
