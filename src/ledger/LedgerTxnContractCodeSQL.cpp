...
{
    ZoneScoped;
    std::string owner = KeyUtils::toStrKey(key.contractCode().owner);
    int64_t contractID = key.contractCode().contractID;

    std::string body;
    LedgerEntry le;
    le.data.type(CONTRACT_CODE);
    ContractCodeEntry& cce = le.data.contractCode();
    ...
    cce.owner = KeyUtils::fromStrKey<PublicKey>(owner);
    cce.contractID = contractID;

    fromOpaqueBase64(cce.body, body);
    ...
    : public DatabaseTypeSpecificOperation<void>
{
    ...
    {
        releaseAssert(entry.data.type() == CONTRACT_CODE);
        ContractCodeEntry const& code = entry.data.contractCode();
        mOwners.emplace_back(KeyUtils::toStrKey(code.owner));
        mContractIDs.emplace_back(code.contractID);
        mContractBodies.emplace_back(toOpaqueBase64(code.body));
  ...
  public:
    ...
    doSociGenericOperation()
    {
        ...
        {
            throw std::runtime_error("Could not update contractcode in SQL");
        }
    }
    ...
    : public DatabaseTypeSpecificOperation<void>
{
  ...
  public:
        ...
        : mDB(DB), mCons(cons)
    {
        for (auto const& e : entries)
        {
            ...
            releaseAssert(e.key().type() ==
                          InternalLedgerEntryType::LEDGER_ENTRY);
            releaseAssert(e.key().ledgerKey().type() == CONTRACT_CODE);
            auto const& code = e.key().ledgerKey().contractCode();
            mOwners.emplace_back(KeyUtils::toStrKey(code.owner));
            mContractIDs.emplace_back(code.contractID);
        }
    }
    ...
    doSociGenericOperation()
    {
        std::string sql = "DELETE FROM contractcode WHERE owner = :owner AND "
    ...
    : public DatabaseTypeSpecificOperation<std::vector<LedgerEntry>>
{
  ...
  public:
        ...
        : mDb(db)
    {
        ...
        {
            releaseAssert(k.type() == CONTRACT_CODE);
            mOwners.emplace_back(KeyUtils::toStrKey(k.contractCode().owner));
    ...
    doSqliteSpecificOperation(soci::sqlite3_session_backend* sq) override
    {
        ...
        std::string sql = "WITH r AS (" + sqlJoin +
                          ...
                          "lastmodified "
                          "FROM contractcode WHERE (owner, contractid) IN r";
...
LedgerTxnRoot::Impl::dropContractCode()
{
    ...
    {
        mDatabase.getSession() << "ALTER TABLE contractcode "
                               << "ALTER COLUMN owner "
