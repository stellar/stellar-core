...
TEST_CASE_VERSIONS("meta stream contains reasonable meta", "[ledgerclosemeta]")
{
...

    {
        ...
        // Ledger #2 creates the account, which we do directly so emit no
        // meta for.
        auto acc1 = root.create("acc1", bal);

        // Ledger 3 uses closeLedger so emits interesting meta.
        std::vector<TransactionFrameBasePtr> txs = {
            root.tx({payment(acc1.getPublicKey(), 1000)})};
        closeLedger(*app, txs);
    }

    // We're going to examine the meta generated by ledger #3.
    uint32_t const targetSeq = 3;
    ...
    while (in.readOne(lcm))
    {
                                    ...
                                    GENERALIZED_TX_SET_PROTOCOL_VERSION))
        {
            ...
            {
                REQUIRE(lcm.v0().txProcessing.size() == 1);
                REQUIRE(lcm.v0().txProcessing.at(0).txApplyProcessing.v() == 2);
                auto const& t2 =
                    lcm.v0().txProcessing.at(0).txApplyProcessing.v2();
                txRes = lcm.v0().txProcessing.at(0).result.result;
                opMetas = t2.operations;
            }
        }
#ifdef ENABLE_NEXT_PROTOCOL_VERSION_UNSAFE_FOR_PRODUCTION
                     ...
                     SOROBAN_PROTOCOL_VERSION))
        {
            ...
            if (ledgerSeq == targetSeq)
            {
                REQUIRE(lcm.v2().txProcessing.size() == 1);
                REQUIRE(lcm.v2().txProcessing.at(0).txApplyProcessing.v() == 3);
                auto const& t3 =
                    lcm.v2().txProcessing.at(0).txApplyProcessing.v3();
                txRes = t3.txResult;
                opMetas = t3.operations;
            }
        }
#endif
        else
        {
            ...
            if (ledgerSeq == targetSeq)
            {
                ...
                REQUIRE(lcm.v1().txProcessing.at(0).txApplyProcessing.v() == 2);
                auto const& t2 =
                    lcm.v0().txProcessing.at(0).txApplyProcessing.v2();
                txRes = lcm.v0().txProcessing.at(0).result.result;
                opMetas = t2.operations;
        ...
        if (ledgerSeq == targetSeq)
        {
            // We should have a single result with payment success
            REQUIRE(txRes.result.code() == txSUCCESS);
            REQUIRE(txRes.result.results().size() == 1);
            OperationResult& ores = txRes.result.results().at(0);
            REQUIRE(ores.code() == opINNER);
            REQUIRE(ores.tr().type() == PAYMENT);
            REQUIRE(ores.tr().paymentResult().code() == PAYMENT_SUCCESS);

            // We should have 1 opMeta with 4 changes: "state(root),
            // updated(root), state(acc1), updated(acc1)"
            REQUIRE(opMetas.size() == 1);
            REQUIRE(opMetas.at(0).changes.size() == 4);
            size_t nState{0}, nUpdate{0};
            for (auto const& ch : opMetas.at(0).changes)
            {
                switch (ch.type())
                {
