...
void
storeHeaderInDatabase(Database& db, LedgerHeader const& header)
{
    if (!isLedgerHeaderValid(header))
    {
...
LedgerHeader
decodeLedgerHeaderFromData(std::string const& data)
{
    LedgerHeader lh;
...
loadLedgerHeaderByHash(Database& db, Hash const& hash)
{
    ...
    if (st.got_data())
    {
        auto lh = decodeLedgerHeaderFromData(headerEncoded);
...
void
deleteOldLedgerHeaders(Database& db, uint32_t ledgerSeq, uint32_t count)
{
        ...
        uint32_t ledgerCount, XDROutputFileStream& headersOut)
{
    ...
    while (st.got_data())
    {
        ...
        lhe.header = decodeLedgerHeaderFromData(headerEncoded);
