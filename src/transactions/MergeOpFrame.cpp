...
MergeOpFrame::doApply(LedgerDelta& delta, LedgerManagerImpl& ledgerMaster)
{
    ...
    if (!AccountFrame::loadAccount(mOperation.body.destination(), otherAccount,
                                   db))
    {
        innerResult().code(AccountMerge::ACCOUNT_MERGE_NO_ACCOUNT);
        return false;
    }
    ...
    {
        innerResult().code(AccountMerge::ACCOUNT_MERGE_HAS_CREDIT);
        return false;
    }
    ...
    innerResult().code(AccountMerge::ACCOUNT_MERGE_SUCCESS);
    return true;
}
...
MergeOpFrame::doCheckValid(Application& app)
{
    ...
    {
        innerResult().code(AccountMerge::ACCOUNT_MERGE_MALFORMED);
        return false;
    }
    return true;
}
