    ...
    std::optional<uint32_t> validationLedgerSeq) const
{
    ...
    if (commonValid(signatureChecker, lrv, false, *txResult) !=
...
FeeBumpTransactionFrame::commonValidPreSeqNum(
    LedgerReadView const& lrv, MutableTransactionResultBase& txResult) const
{
    ...
    auto feeSource = lrv.getAccount(getFeeSourceID());
    if (!feeSource)
    {
    ...
    bool applying, MutableTransactionResultBase& txResult) const
{
    ...
    // account loading
    auto feeSource = commonValidPreSeqNum(lrv, txResult);
    if (!feeSource)
    {
        return res;
    }
