    ...
    std::vector<SorobanApplyMetrics>& sorobanApplyMetricsPerThread)
{
    ...
    //
    // This phase separation hinges on the fact that the validation outcome
    ...
    //   because any given transaction in a ledger is unique, and decreasing the
    ...
    std::vector<SorobanApplyMetrics>& sorobanApplyMetricsPerThread)
{
    ...
    // Look up the newest version of the entry updated so far in this ledger,
    // if any. This only uses `LedgerTxn::getNewestVersionBelowRoot()`, which is
    // thread safe as long as we don't mutate the ltx.
    auto getUpdatedEntry = [&ltx](LedgerKey const& key)
        -> std::optional<std::shared_ptr<LedgerEntry const>> {
        auto [isUpdated, entry] = ltx.getNewestVersionBelowRoot(key);
        if (!isUpdated)
        {
            return std::nullopt;
        }
        if (!entry)
        {
            // The entry has been deleted in this ledger.
        ...
        return std::shared_ptr<LedgerEntry const>(entry, &entry->ledgerEntry());
    };
