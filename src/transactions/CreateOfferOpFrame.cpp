...
CreateOfferOpFrame::checkOfferValid(Database& db)
{
    ...
    if (sheep.type() != NATIVE)
    {
        ...
        { // we don't have what we are trying to sell
            innerResult().code(CreateOffer::CREATE_OFFER_NO_TRUST);
            return false;
        }
        if (mSheepLineA.getBalance() == 0)
        {
            innerResult().code(CreateOffer::CREATE_OFFER_UNDERFUNDED);
            return false;
        }
    }
    ...
    if (wheat.type() != NATIVE)
    {
        ...
        { // we are not authorized to hold what we are trying to buy
            innerResult().code(CreateOffer::CREATE_OFFER_NOT_AUTHORIZED);
            return false;
        }
    }
    return true;
...
CreateOfferOpFrame::doApply(LedgerDelta& delta, LedgerManagerImpl& ledgerMaster)
{
    ...
    { // modifying an old offer
        if (OfferFrame::loadOffer(getSourceID(), offerID, mSellSheepOffer, db))
        {
            ...
            {
                innerResult().code(CreateOffer::CREATE_OFFER_MALFORMED);
                return false;
            }
        }
        else
        {
            innerResult().code(CreateOffer::CREATE_OFFER_NOT_FOUND);
            return false;
        }
    }
    else
    ...
    innerResult().code(CreateOffer::CREATE_OFFER_SUCCESS);

    {
        ...
        OfferExchange::ConvertResult r = oe.convertWithOffers(
            ...
            [this, maxWheatPrice](OfferFrame const& o)
            {
                ...
                if (o.getAccountID() == getSourceID())
                {
                    ...
                    innerResult().code(CreateOffer::CREATE_OFFER_CROSS_SELF);
                    return OfferExchange::eStop;
                }
        ...
        case OfferExchange::eFilterStop:
            if (innerResult().code() != CreateOffer::CREATE_OFFER_SUCCESS)
            {
                return false;
            }
            offerIsValid = true;
        ...
        { // we still have sheep to sell so leave an offer
            ...
            if (creatingNewOffer)
            {
                ...
                innerResult().success().offer.effect(CreateOffer::CREATE_OFFER_CREATED);
            ...
            {
                innerResult().success().offer.effect(CreateOffer::CREATE_OFFER_UPDATED);
                mSellSheepOffer.storeChange(tempDelta, db);
            }
        }
        else
        {
            innerResult().success().offer.effect(CreateOffer::CREATE_OFFER_EMPTY);
