...
PaymentOpFrame::doApply(LedgerDelta& delta, LedgerManagerImpl& ledgerMaster)
{
    ...
    {
        innerResult().code(Payment::PAYMENT_SUCCESS);
        return true;
    }
    ...
    { // this tx is creating an account
        if (mPayment.currency.type() == NATIVE)
        {
            ...
            { // not over the minBalance to make an account
                innerResult().code(Payment::PAYMENT_UNDERFUNDED);
                return false;
            }
            else
            {
        ...
        { // trying to send credit to an unmade account
            innerResult().code(Payment::PAYMENT_NO_DESTINATION);
            return false;
        }
    }
                             ...
                             LedgerManagerImpl& ledgerMaster)
{
    ...
    if (multi_mode)
    {
        innerResult().code(Payment::PAYMENT_SUCCESS_MULTI);
    }
    else
    {
        innerResult().code(Payment::PAYMENT_SUCCESS);
    ...
    // update last balance in the chain
    {
        ...
        else if (destination.getID() != curB.isoCI().issuer)
        {
            ...
            if (!TrustFrame::loadTrustLine(destination.getID(), curB, destLine,
                                           db))
            {
                innerResult().code(Payment::PAYMENT_NO_TRUST);
                return false;
            }

            if (!destLine.getTrustLine().authorized)
            {
                innerResult().code(Payment::PAYMENT_NOT_AUTHORIZED);
                return false;
            }

            if (!destLine.addBalance(curBReceived))
            {
                innerResult().code(Payment::PAYMENT_LINE_FULL);
                return false;
            }
    ...
    if (multi_mode)
    {
        ...
        for (int i = (int)mPayment.path.size() - 1; i >= 0; i--)
        {
            ...
            case OfferExchange::ePartial:
                innerResult().code(Payment::PAYMENT_OVERSENDMAX);
                return false;
            }
    ...
    if (curB.type() == NATIVE)
    {
        ...
        {
            innerResult().code(Payment::PAYMENT_MALFORMED);
            return false;
        }
