...
TEST_CASE("authorized to maintain liabilities", "[tx][allowtrust]")
{
    ...
    auto offerTest = [&](bool buyIsOnlyAllowedToMaintainLiabilities) {
        ...
        SECTION("allow cross offer")
        {
            a2.changeTrust(usd, trustLineLimit);
            gateway.allowTrust(usd, a2);
            ...
            market.requireBalances(
                {{a1,
                  {{usd, trustLineStartingBalance},
                   {idr, trustLineStartingBalance}}},
                 {a2, {{usd, 0}, {idr, trustLineStartingBalance}}}});

            SECTION("taking order has greater size and is posted to book")
            {
                market.requireChangesWithOffer(
                    {{offer.key, OfferState::DELETED}}, [&] {
                        return market.addOffer(a2,
                                               {idr, usd, Price{1, 1}, 1001},
                                               {idr, usd, Price{1, 1}, 1});
                    });

                market.requireBalances(
                    {{a1,
                      {{usd, trustLineStartingBalance - 1000},
                       {idr, trustLineStartingBalance + 1000}}},
                     {a2,
                      {{usd, 1000}, {idr, trustLineStartingBalance - 1000}}}});
            }

            SECTION("making order has greater size and stays in book")
            {
                market.requireChangesWithOffer(
                    {{offer.key, {usd, idr, Price{1, 1}, 1}}}, [&] {
                        return market.addOffer(a2, {idr, usd, Price{1, 1}, 999},
                                               OfferState::DELETED);
                    });

                market.requireBalances(
                    {{a1,
                      {{usd, trustLineStartingBalance - 999},
                       {idr, trustLineStartingBalance + 999}}},
                     {a2,
                      {{usd, 999}, {idr, trustLineStartingBalance - 999}}}});
            }

            SECTION("both orders have same size")
            {
                market.requireChangesWithOffer(
                    {{offer.key, OfferState::DELETED}}, [&] {
                        return market.addOffer(a2,
                                               {idr, usd, Price{1, 1}, 1000},
                                               OfferState::DELETED);
                    });
