...
TEST_CASE("payment", "[tx][payment]")
{
    ...
    SECTION("simple credit")
    {
        ...
        {
            REQUIRE_THROWS_AS(gateway.pay(getAccount("B").getPublicKey(), idrCur, 100), ex_PAYMENT_NO_DESTINATION);
        }

        // actual sendcredit
        SECTION("credit payment with no trust")
        {
            REQUIRE_THROWS_AS(gateway.pay(a1, idrCur, 100), ex_PAYMENT_NO_TRUST);
        }
        ...
        SECTION("protocol version " + std::to_string(v))
        {
            ...
            SECTION("payment through path")
            {
                ...
                SECTION("send with path (over sendmax)")
                {
                    ...
                    REQUIRE_THROWS_AS(a1.pay(b1, usdCur, 149 * assetMultiplier, idrCur,
                                            100 * assetMultiplier, {}),
                ...
                SECTION("missing issuer")
                {
                    SECTION("dest is standard account")
                    {
                        SECTION("last")
                        {
                            ...
                            REQUIRE_THROWS_AS(a1.pay(b1, usdCur, 250 * assetMultiplier, idrCur,
                                                     125 * assetMultiplier, {}, &idrCur),
                        ...
                        SECTION("mid")
                        {
                            ...
                            REQUIRE_THROWS_AS(a1.pay(b1, usdCur, 250 * assetMultiplier, idrCur,
                                                     125 * assetMultiplier, {btcCur}, &btcCur),
                                              ex_PATH_PAYMENT_NO_ISSUER);
                        }
                    }
                    SECTION("dest is issuer")
                    {
                        ...
                        SECTION("cannot take offers on the way")
                        {
                            ...
                            REQUIRE_THROWS_AS(a1.pay(gateway, usdCur, 250 * assetMultiplier, idrCur,
                                                     125 * assetMultiplier, {}),
                ...
                SECTION("send with path (offer participant reaching limit)")
                {
                    ...
                    auto res = a1.pay(b1, usdCur, 400 * assetMultiplier, idrCur, 105 * assetMultiplier, {});
                ...
                SECTION("missing trust line")
                {
                    ...
                    auto checkBalances = [&]()
                    {
                        auto res = a1.pay(b1, usdCur, 200 * assetMultiplier, idrCur,
                                          25 * assetMultiplier, {});
            ...
            SECTION("path payment with rounding errors")
            {
                ...
                if (app.getLedgerManager().getCurrentLedgerVersion() <= 2)
                {
                    ...
                    REQUIRE_THROWS_AS(source.pay(destination, xlmCur, 1382068965, cnyCur, 2 * assetMultiplier, path),
                                      ex_PATH_PAYMENT_TOO_FEW_OFFERS);
                }
                else
                {
                    source.pay(destination, xlmCur, 1382068965, cnyCur, 2 * assetMultiplier, path);
                        ...
                        for (size_t i = 0; i < pathSize; i++)
                        {
                            ...
                            SECTION(std::string{"send with path ("} + assetPathToString(assets) + ")")
                            {
                                ...
                                for (size_t j = 0; j < pathSize; j++)
                                {
                                    ...
                                    validateAccountAsset(accounts[index], index, -paymentAmount * sellerMultipler, 3);                // sold asset
                                    validateOffer(accounts[index], offers[index], -paymentAmount * sellerMultipler);                  // sold asset
                                    validateAccountAsset(accounts[index], (index + 2) % pathSize, paymentAmount * buyerMultipler, 3); // bought asset
                                    validateAccountAsset(accounts[index], (index + 1) % pathSize, 0, 3);                              // ignored asset
