...
`stream-priority.patch` contains all changes to upstream source, in nine files:

- `libp2p-quic` delegates priority set/get to `quinn::SendStream`.
- `libp2p-core` carries optional priority callbacks through muxer type erasure,
...
Run `cargo test --locked -p stellar-overlay`. Tests use real libp2p/QUIC peers
...
`overlay/vendor`. Restore each license from its recorded URL. To verify an
existing copy, `patch --dry-run -R -p1 < stream-priority.patch` checks that the
patch can be reversed; compare the resulting source to the verified archive
